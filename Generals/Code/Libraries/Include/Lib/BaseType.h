...
typedef __int64_t Int64;		  // 8 bytes
typedef __uint64_t UnsignedInt64; // 8 bytes
...
__forceinline long fast_float2long_round(float f)
{
	...
	__asm__(
		"flds %1\n\t" // Load the float value from 'f' into the FPU stack
		"fistpl %0"	  // Convert to integer and store in 'i'
		: "=m"(i)	  // Output operand: memory location for 'i'
		: "m"(f)	  // Input operand: memory location for 'f'
		: "st"		  // Clobber list: indicates the FPU stack is used
	);
